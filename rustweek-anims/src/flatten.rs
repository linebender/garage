...
use vello::kurbo;
use vello::kurbo::{Affine, BezPath, Stroke, StrokeOpts};
...
// Simplified version of fill, for animations
pub fn from_kurbo(path: &BezPath, affine: Affine, line_buf: &mut Vec<Line>) {
    let mut p0 = kurbo::Point::default();
    for el in path.iter().map(|el| affine * el) {
        match el {
            kurbo::PathEl::MoveTo(p) => p0 = p,
            kurbo::PathEl::LineTo(p) => {
                let pt0 = Point::new(p0.x as f32, p0.y as f32);
                let pt1 = Point::new(p.x as f32, p.y as f32);
                line_buf.push(Line::new(pt0, pt1));
