...
impl LogAestheticParams {
    ...
    fn integrate_multiple(&self) -> (Vec<Vec2>, Vec2) {
        ...
        let integrals = (0..n)
            .map(|i| {
                let piece = self.integrate(i as f64 * dt..(i + 1) as f64 * dt);
...
impl LogAestheticCurve {
    ...
    pub fn from_points_params(params: LogAestheticParams, p0: Point, p1: Point) -> Self {
        let (integrals, uv) = params.integrate_multiple();
    ...
    /// Create a log-aesthetic curve from parameters and arc length.
    ///
    /// The tangent at s=0 is fixed.
    pub fn from_params_arclen(params: LogAestheticParams, p0: Point, arclen: f64) -> Self {
        let (integrals, uv) = params.integrate_multiple();
        let scale_rot = Vec2::new(arclen, 0.0);
        let p1 = p0 + arclen * uv;
        LogAestheticCurve {
            params,
            p0,
